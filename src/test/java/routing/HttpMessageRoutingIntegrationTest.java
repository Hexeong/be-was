...
public class HttpMessageRoutingIntegrationTest {
    ...
    void 동적_컨텐츠_요청에_정상적으로_작동한다() {
        // given - [AI]
        Map<String, Object> createUserParams = Map.of(
                "userId", "javajigi",
                "password", "password",
                "name", "JaeSung",
        ...
        assertThat(savedUser.getUserId()).isEqualTo(createUserParams.get("userId"));
        assertThat(savedUser.getPassword()).isEqualTo(createUserParams.get("password"));
        assertThat(savedUser.getName()).isEqualTo(createUserParams.get("name"));
        assertThat(savedUser.getEmail()).isEqualTo(createUserParams.get("email"));
    }
